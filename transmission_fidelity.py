...
for i in range(1000):
    ...
    r = np.random.random()
    # if r < rho1, new seed trait is introduced
    if r < rho1:
    ...
    # if r < rho1 + rho2, two of the cultural traits present are combined
    elif r < rho1 + rho2:
        # draw two random traits from culture group
        trait_1 = np.random.choice(culture_group, 1, replace=False)
        remaining_traits = np.setdiff1d(culture_group, trait_1)
        trait_2 = np.random.choice(remaining_traits, 1, replace=False)
        # combine traits if not same seed trait
        #if trait_1[0] != trait_2[0]:
